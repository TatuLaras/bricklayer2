...
static VkDeviceMemory index_memory;
...
static int framebuffer_resized = 0;
...
static inline void create_swapchain(void) {
...
static inline void create_buffer(VkDeviceSize size,
    ...
    uint32_t selected_memory_type =
        find_memory_type(memory_requirements, memory_property_flags);
...
static inline void begin_single_time_commands(VkCommandBuffer *command_buffer) {
...
static inline void end_single_time_commands(VkCommandBuffer command_buffer) {
...
static inline void create_depth_resources(void) {
...
static inline void create_texture(void) {
    ...
    VK_ERR(vkCreateImageView(
        logical_device, &image_view_create_info, NULL, &texture_image_view));
    ...
    VK_ERR(
        vkCreateSampler(logical_device, &sampler_create_info, NULL, &sampler));
}
...
static inline void
transition_swapchain_image_layout(uint32_t image_index,
                                  VkImageLayout old_layout,
                                  VkImageLayout new_layout,
...
static inline void update_uniform_buffer(uint32_t i) {
    ...
    // glm_rotate(ubo_data.model, rotation, right);
...
static inline void cleanup(void) {
    ...
    vkDestroyImage(logical_device, texture_image, NULL);
    vkFreeMemory(logical_device, texture_image_memory, NULL);
    vkDestroyImageView(logical_device, texture_image_view, NULL);
    vkDestroySampler(logical_device, sampler, NULL);
...
int old_main(int argc, char **argv) {
    ...
    upload_data(mesh.vertices,
                mesh.vertex_count * sizeof *mesh.vertices,
                VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                &vertex_buffer,
                &vertex_memory);
    upload_data(mesh.indices,
